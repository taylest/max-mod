...
if (window.navigator && !window.navigator.share) {
    window.navigator.share = data => {
        const PERMITTED_FILE_SIZE = 1024 * 1024 * 3;
        ...
        const { url, title, text } = data
                ...
                if (file && file.arrayBuffer && file.size < PERMITTED_FILE_SIZE) {
                    file.arrayBuffer().then(buffer => {
                        const bytes = Array.from(new Uint8Array(buffer))
                        const filename = file.name
                        const filetype = file.type
                        window.WebViewHandler.resolveShare(JSON.stringify({ url, title, text }), bytes, filename, filetype);
                    })
                } else {
                    window.WebViewHandler.resolveShare(JSON.stringify({ url, title, text }), null, null, null);
                }
